...
`postService.ts`는 DB → 파일시스템 순서로 fallback한다.

```
getAllPosts() / getPostBySlug()
  1. articleRepository (DB) 시도
  2. DB에 결과 없거나 오류 → 빈 배열 반환
```

DB가 없거나 연결 실패 시에도 파일시스템 기반으로 공개 블로그가 작동한다. `BlogPost.contentSource` 필드로 출처 구분 가능(`'database'` | `'filesystem'`).
...
| 타입 | 설명 |
|------|------|
| `BlogPost` | 공개 블로그 글. `contentSource`로 DB/파일 출처 구분 |
